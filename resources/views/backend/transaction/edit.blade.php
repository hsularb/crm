        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('transaction.update',$data->id) }}" autocomplete="off"
                            enctype="multipart/form-data">
                            @csrf
                            @method('PATCH')
                            <h6 class="heading-small text-muted mb-4">{{ __('Edit Transaction') }}</h6>
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="income_source_id">
                                        ...
                                        @foreach($income as $row)
                                            <option value="{{$row->id}}" {{$data->income_source_id==$row->id ? 'selected':''}}>{{$row->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="amount" id="input-name" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Amount') }}" value="{{ old('name', $data->amount) }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="currency_id">
                                        ...
                                        @foreach($currency as $row)
                                            <option value="{{$row->id}}" {{$data->currency_id==$row->id ? 'selected':''}}>{{$row->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="transaction_type_id">
                                        ...
                                        @foreach($type as $row)
                                            <option value="{{$row->id}}" {{$data->transaction_type_id==$row->id ? 'selected':''}}>{{$row->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="date" name="transaction_date" id="input-name" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Transaction Date') }}" value="{{ $data->transaction_date }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <textarea type="text" name="description" id="input-name" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Description') }}" value="{{ old('name', $data->description) }}" required autofocus>{{$data->description}}</textarea>
